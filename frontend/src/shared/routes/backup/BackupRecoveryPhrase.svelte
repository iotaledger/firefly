...
{:else}
    <OnboardingLayout allowBack={true}>
        <div slot="leftpane__content">
            ...
            <Text type="p" secondary={true} classes="mb-8">{locale('views.backup-recovery-phrase.body')}</Text>
            <Password
                classes="mb-6"
                bind:value={password}
                strengthLevels={4}
        ...
        <div slot="leftpane__action" class="flex flex-row justify-end items-center">
            <Button disabled={!valid} onClick={() => goto('recovery-phrase-saved')}>{locale('actions.save_password')}</Button>
        </div>
