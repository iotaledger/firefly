...
use crate::noise_xx::{consts::{HASHLEN, MAC_LENGTH, MAX_MESSAGE},
			error::NoiseError,
...
impl NoiseSession {
	...
	pub fn _is_transport(&self) -> bool {
		self.is_transport
	}

	/// Clears `cs1`.
	pub fn _clear_local_cipherstate(&mut self) {
		self.cs1._clear();
	}

	/// Clears `cs2`.
	pub fn _clear_remote_cipherstate(&mut self) {
		self.cs2._clear();
	}

	/// Calls the [Rekey](https://noiseprotocol.org/noise.html#rekey) method for `cs1`
	pub fn _rekey_local_cipherstate(&mut self) {
		self.cs1.rekey()
	}

	/// Calls the [Rekey](https://noiseprotocol.org/noise.html#rekey) method for `cs2`
	pub fn _rekey_remote_cipherstate(&mut self) {
		self.cs1.rekey()
	}
	
	/// `NoiseSession` destructor function.
	pub fn _end_session(mut self) {
		self.hs.clear();
		self._clear_local_cipherstate();
		self._clear_remote_cipherstate();
		self.cs2._clear();
		self.mc = 0;
		self.h = Hash::new();
	}
	...
	/// Returns `h`.
	pub fn _get_handshake_hash(&self) -> Option<[u8; HASHLEN]> {
	...
	/// Returns `mc`.
	pub fn _get_message_count(&self) -> u128 {
		self.mc
	}
	...
	pub fn _get_remote_static_public_key(&self) -> Option<PublicKey> {
		if self.is_transport {
			return Some(self.hs._get_remote_static_public_key());
		}
		None
	}
