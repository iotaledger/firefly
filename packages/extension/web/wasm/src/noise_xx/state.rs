...
use crate::noise_xx::{consts::{DHLEN, EMPTY_HASH, EMPTY_KEY, HASHLEN, MAC_LENGTH, NONCE_LENGTH, ZEROLEN},
			error::NoiseError,
...
impl CipherState {
	...
	pub(crate) fn encrypt_with_ad(
		...
		if self.has_key() {
			...
			encrypt(from_slice_hashlen(&self.k.as_bytes()[..]), nonce, ad, in_out, &mut ciphertext[..], mac);
	...
	pub(crate) fn decrypt_with_ad(
		...
		if self.has_key() {
			...
			if decrypt(from_slice_hashlen(&self.k.as_bytes()[..]), nonce, ad, &mut plaintext[..], in_out, mac) {
