    ...
    match serde_json::from_str::<DispatchMessage>(msg) {
        Ok(v)=>{
            ...
            match v.message {
                ...
                MessageType::CallAccountMethod{account_id:_account_id,method:_method}=>{
                    final_message = msg.to_string();
                },
                MessageType::SendTransfer{account_id:_account_id,transfer:_transfer}=>{
                    final_message = msg.to_string();
                },
                MessageType::CallGlow{method, payload:_}=>{
                    ...
                    if method=="CheckLink" {
                        ...
                        let res = serde_json::to_string(&Response::new(
                            v.id, m, ResponseType::CalledGlow("INITIALIZED".to_string()),
                        ))
                        .unwrap();
                        return (Ok(res), true)
                    }
