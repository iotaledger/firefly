<script lang="typescript">
    ...
    import { Box, OnboardingLayout, Illustration, Text, Button } from 'shared/components'
...
{:else}
    <OnboardingLayout allowBack={false}>
        <div slot="leftpane__content">
            <Box classes="bg-gray-50 dark:bg-gray-900 dark:bg-opacity-50 rounded-lg p-10">
                <balance class="flex flex-col flex-grow items-center">
                    <Text type="h1" classes="mb-5 text-center">{locale('views.congratulations.title')}</Text>
                    <Text type="p" secondary classes="mb-4 text-center">{locale('views.congratulations.body')}</Text>
        ...
        <div slot="leftpane__action">
            <Button classes="w-full" onClick={() => handleContinueClick()}>{locale('views.congratulations.export_migration')}</Button>
        </div>
