                ...
                {#each networkConfig.nodes as node}
                    ...
                    >
                        <div class="flex flex-row items-center space-x-4 overflow-hidden">
                            ...
                            <Text highlighted classes="{$mobile && node.isPrimary && 'absolute right-5 p-1 rounded-lg bg-pastel-green'}">
                                {node.isPrimary && !$mobile ? localize('views.settings.configureNodeList.primaryNode') : ''}
                            </Text>
                        </div>
                        {#if !$mobile}
                            <button
