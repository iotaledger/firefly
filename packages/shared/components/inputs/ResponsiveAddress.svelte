<script lang="typescript">
    ...
    export let onAddressChange: (address: string) => void
    ...
    function onBlur() {
        hasFocus = false
        address = responsiveAddress
        shortenAddress(responsiveAddress)
    }

    function onFocus() {
        hasFocus = true
        responsiveAddress = address
    }

    function onInput() {
        if (onAddressChange) {
            onAddressChange(responsiveAddress)
        }
        if (hasFocus === false) {
            return
        }
    ...
    function shortenAddress(_address) {
        if (_address.length <= maxLength) {
            responsiveAddress = _address
            return
        }
        const start = _address.slice(0, maxLength / 2)
        const end = _address.slice(_address.length - maxLength / 2 + 3, _address.length)
        responsiveAddress = `${start}...${end}`
    }

    $: shortenAddress(address)

    $: responsiveAddress, onInput()
</script>
