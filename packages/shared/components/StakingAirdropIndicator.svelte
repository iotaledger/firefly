...
{#if showIndicator}
    ...
    >
        <span class="ml-4 absolute flex justify-center items-center h-3 w-3">
            {#if isStaked}
                <span id="indicator" class="animate--ping absolute inline-flex h-full w-full rounded-full bg-{isStaked ? 'green' : 'red'}-400 opacity-75"></span>
            {/if}
        ...
        >
            <Text type="p" classes="text-white dark:text-white">{localize(`general.${isStaked ? 'staked' : 'unstaked'}`)}</Text>
        </div>
    </div>
{/if}
