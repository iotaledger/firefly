...
{#if recoveryPhrase}
    <div
        ...
        {#each recoveryPhrase as word, i}
            <span
                id="recovery-word-{i}"
                class="px-6 py-4 flex flex-row items-center rounded-2xl {$mobile ? 'bg-transparent text-gray-500 border border-solid border-gray-300' : 'bg-gray-200 dark:bg-gray-800'}"
