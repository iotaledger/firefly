...
export function sumTotalClaimedRewards(shimmerClaimingAccounts: IShimmerClaimingAccount[]): number {
    return shimmerClaimingAccounts?.reduce((total: number, curr: IShimmerClaimingAccount) => (
            total +
            tryNumberOrZero(
                curr?.claimingTransaction?.payload?.essence?.outputs?.reduce(
                    (outputsTotal: number, currentOutput: OutputTypes) => outputsTotal + tryNumberOrZero(currentOutput?.amount),
                    0
                )
            )
        ), 0)
