/**
 * The icons available in the app.
 */
export enum Icon {
    Activity = 'activity',
    Alias = 'alias',
    ArrowLeft = 'arrow-left',
    ArrowRight = 'arrow-right',
    Assembly = 'assembly',
    Backspace = 'backspace',
    Bell = 'bell',
    Biometric = 'biometric',
    Calendar = 'calendar',
    Checkbox = 'checkbox',
    CheckboxRound = 'checkbox-round',
    CheckboxUnchecked = 'checkbox-unchecked',
    CheckboxUncheckedRound = 'checkbox-unchecked-round',
    Checkmark = 'checkmark',
    CheckmarkFilled = 'checkmark-filled',
    ChevronDown = 'chevron-down',
    ChevronLeft = 'chevron-left',
    ChevronRight = 'chevron-right',
    ChevronUp = 'chevron-up',
    Chip = 'chip',
    Close = 'close',
    CollectiblesApplicationPlaceholder = 'collectibles-application-placeholder',
    CollectiblesAudioPlaceholder = 'collectibles-audio-placeholder',
    CollectiblesFontPlaceholder = 'collectibles-font-placeholder',
    CollectiblesImagePlaceholder = 'collectibles-image-placeholder',
    CollectiblesModelPlaceholder = 'collectibles-model-placeholder',
    CollectiblesTextPlaceholder = 'collectibles-text-placeholder',
    CollectiblesUnknownPlaceholder = 'collectibles-unknown-placeholder',
    CollectiblesVideoPlaceholder = 'collectibles-video-placeholder',
    Collectibles = 'collectibles',
    Copy = 'copy',
    Currency = 'currency',
    Customize = 'customize',
    Delete = 'delete',
    Dev = 'dev',
    Discord = 'discord',
    Doc = 'doc',
    DoubleChevronRight = 'double-chevron-right',
    Edit = 'edit',
    ErrorFilled = 'error-filled',
    ExclamationFilled = 'exclamation-filled',
    Exclamation = 'exclamation',
    ExpirationTime = 'expiration-time',
    Export = 'export',
    File = 'file',
    Filter = 'filter',
    Firefly = 'firefly',
    Governance = 'governance',
    Help = 'help',
    Hide = 'hide',
    History = 'history',
    Info = 'info',
    InfoFilled = 'info-filled',
    Iota = 'iota',
    Language = 'language',
    Ledger = 'ledger',
    LedgerApp = 'ledger-app',
    LedgerAppLegacy = 'ledger-app-legacy',
    Link = 'link',
    Lock = 'lock',
    Lock2 = 'lock2',
    Lock3 = 'lock3',
    Logout = 'logout',
    Minus = 'minus',
    Network = 'network',
    NotVerified = 'not-verified',
    Parchment = 'parchment',
    Picture = 'picture',
    Play = 'play',
    Plus = 'plus',
    Profile = 'profile',
    Qr = 'qr',
    Radio = 'radio',
    RadioUnchecked = 'radio-unchecked',
    Receive = 'receive',
    Refresh = 'refresh',
    Reset = 'reset',
    Search = 'search',
    Security = 'security',
    Seed = 'seed',
    Send = 'send',
    Settings = 'settings',
    Shield = 'shield',
    Shimmer = 'shimmer',
    SmallChevronDown = 'small-chevron-down',
    SmallChevronLeft = 'small-chevron-left',
    SmallChevronRight = 'small-chevron-right',
    SmallChevronUp = 'small-chevron-up',
    Speech = 'speech',
    Staking = 'staking',
    StatusError = 'status-error',
    StatusSuccess = 'status-success',
    SuccessCheck = 'success-check',
    Sync = 'sync',
    Theme = 'theme',
    ThemeDark = 'theme-dark',
    ThemeLight = 'theme-light',
    Timelock = 'timelock',
    Timer = 'timer',
    Tokens = 'tokens',
    Tools = 'tools',
    Transfer = 'transfer',
    Unlock = 'unlock',
    VerificationStatusNew = 'verificationStatusNew',
    VerificationStatusOfficial = 'verificationStatusOfficial',
    VerificationStatusSelf = 'verificationStatusSelf',
    View = 'view',
    Voted = 'voted',
    Voting = 'voting',
    Wallet = 'wallet',
    Warning = 'warning',
    WarningFilled = 'warning-filled',
    Wifi = 'wifi',
    Work = 'work',
    Verified = 'verified',
}
