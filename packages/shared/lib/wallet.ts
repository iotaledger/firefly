...
export const asyncCreateAccount = () => {
    return new Promise<void>((resolve, reject) => {
        const networkId = getOfficialDefaultNetwork()
...
export const buildAccountNetworkSettings = (): {
    ...
    let customNodes = activeProfileSettings?.customNodes ?? []
    let networkId = activeProfileSettings?.networkId ?? getOfficialDefaultNetwork()
    let customNetworkId = activeProfileSettings?.customNetworkId ?? ''
    let actualNetworkId = networkId === 'custom' ? customNetworkId : networkId
    ...
    for (const net of allNetworks) {
        const netNodes = getOfficialNodes(net)

        allOfficialNodes = allOfficialNodes.concat(netNodes)

        if (net === actualNetworkId) {
            officialNodes = netNodes
        }
    }
    ...
    // Iterate through the custom nodes info from the profile
    // and transfer any details to the corresponding node in the list
    ...
    for (const customNode of customNodes) {
        ...
        if (foundNode) {
            ...
            foundNode.isPrimary = customNode.isPrimary

            if (!allNetworks.includes(customNode.networkId)) {
                allNetworks.push(customNode.networkId)
            }
        }
    }
...
export const updateAccountNetworkSettings = async (
    ...
    const customNodes = nodes.filter(n => n.isCustom).map(n => ({
        url: n.url,
        disabled: n.disabled,
    ...
    let actualNetworkId = automaticNodeSelection ? getOfficialDefaultNetwork() : (networkId === 'custom' ? customNetworkId : networkId)
    ...
    if (nonOfficialNodes.length > 0) {
        clientNodes = [...clientNodes, ...nonOfficialNodes.map(o => (
            { 
                ...o, 
                disabled: automaticNodeSelection ? true : o.disabled 
            }))
        ]
    }
