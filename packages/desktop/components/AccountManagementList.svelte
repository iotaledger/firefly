<script lang="ts">
    ...
    import { Height, Pane, TextType, Text, ClickableTile, FontWeight, Pill, Button } from '@ui'
    ...
    function showImplicitAddress(): void {
        openPopup({
            id: PopupId.ImplicitAccountAddress,
        })
    }
</script>
...
<left-pane class="flex flex-col w-1/3">
    <Pane height={Height.Full}>
        <left-pane-container class="flex flex-col space-y-4 h-full justify-between">
            <div class="flex flex-col space-y-4 h-full scrollable-y">
            ...
            <Button onClick={showImplicitAddress} classes="w-full">
                {localize('views.accountManagement.list.newAccount')}
            </Button>
        </left-pane-container>
    </Pane>
</left-pane>
