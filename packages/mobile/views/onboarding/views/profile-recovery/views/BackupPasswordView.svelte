<script lang="typescript">
    ...
    let title = `${localize('general.import')} ${localize(`general.${$onboardingProfile?.recoveryType}`)}`
...
<OnboardingLayout {onBackClick} {busy} {title} animation="import-from-file-password-desktop">
    ...
    <div style={$isKeyboardOpen && `margin-bottom: ${$keyboardHeight}px`} slot="footer" class="flex flex-row flex-wrap justify-between items-center space-x-4">
        <Button
            classes="flex-1"
