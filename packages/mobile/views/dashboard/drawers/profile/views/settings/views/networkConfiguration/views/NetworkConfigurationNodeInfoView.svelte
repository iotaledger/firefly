                ...
                {#each Object.keys(NODE_INFO_TAB_MAP[nodeInfoTab]).map( (nodeInfoTabKey) => processNodeInfoMapTab(nodeInfoTab, nodeInfoTabKey) ) as { localeKey, nodeInfoValue }}
                    <CopyableBox value={nodeInfoValue}>
                        <div class="w-full h-full flex flex-row justify-between items-center">
                            <Text type={TextType.p} fontSize="sm" fontWeight={FontWeight.semibold} secondary
                            ...
                            {:else if Array.isArray(nodeInfoValue)}
                                <div class="text-right w-5/6">
                                    {#if nodeInfoValue.length}
                                        <Text type={TextType.p} fontSize="sm" classes="w-full break-words" secondary
                                            >{nodeInfoValue.join(', ')}</Text
                                        >
                                    {:else}
                                        <Text type={TextType.p} fontSize="sm" secondary>{localize('general.none')}</Text
                                        >
                                    {/if}
                                </div>
                            {:else}
                                <Text type={TextType.p} fontSize="sm" secondary>{nodeInfoValue}</Text>
                            {/if}
                        </div>
                    </CopyableBox>
                {/each}
